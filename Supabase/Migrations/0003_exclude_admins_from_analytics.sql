-- 0003 — Exclude admin activity from the analytics overview.
--
-- Recruiter UX analytics should reflect recruiters, not admins inspecting the
-- dashboard. We resolve each event's actor via _recruiters.is_admin (the
-- authoritative flag) and drop admin rows. This makes BOTH the org-wide totals
-- (p_recruiter null) and every per-recruiter view exclude admins, while each
-- non-admin recruiter remains individually viewable.
--
-- Depends on the frontend identifying telemetry by the logged-in recruiter's
-- own email (App.tsx) — otherwise admin events stamped with a recruiter's
-- email can't be told apart. Historical rows written before that fix can't be
-- retroactively re-attributed and remain as-is.
--
-- Adds a function only (create or replace) — no table/column changes. Safe to
-- run and re-run.
--
-- Rollback: re-run 0002_analytics_overview.sql to restore the un-joined base.
...
as $$
  with base as (
    select e.*
    from _telemetry_events e
    left join _recruiters r on lower(r.email) = lower(e.recruiter_email)
    where e.ts >= now() - make_interval(days => greatest(p_days, 1))
      and coalesce(r.is_admin, false) = false           -- drop admin activity
      and (p_recruiter is null or e.recruiter_email = p_recruiter)
  )
